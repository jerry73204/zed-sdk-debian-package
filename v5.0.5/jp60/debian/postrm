#!/bin/sh
# postrm script for zed-sdk-jetpack
...
case "$1" in
    purge)
        ...
        rm -rf /usr/local/zed/settings 2>/dev/null || true
        rm -rf /usr/local/zed/resources 2>/dev/null || true
        
        # Remove AI models (they're large and user-specific)
        if [ -d "/usr/local/zed/resources" ]; then
        ...
        if [ -d "/usr/local/zed" ]; then
            rmdir --ignore-fail-on-non-empty /usr/local/zed 2>/dev/null || true
        fi
        ...
        # Remove systemd service remnants
    ...
    remove|upgrade|failed-upgrade|abort-install|abort-upgrade|disappear)
        ...
        # Reload systemd daemon to clear service definitions
        if command -v systemctl >/dev/null && systemctl list-units >/dev/null 2>&1; then
            systemctl daemon-reload || true
        fi
        
        # Clean up broken symlinks in /usr/local/bin
        for link in /usr/local/bin/ZED_*; do
